...
class SendContactForm extends Controller
{
	...
	* @param Request The incoming Request.
	...
	Log::debug("SendContactForm::__invoke: sending mail to " . config("mail.to"));
	Mail::to(config("mail.to"))->send(new ContactFormSent($validatedData["name"], $validatedData["email"], $validatedData["message"])); // Send the email
